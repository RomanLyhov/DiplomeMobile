    ...
    import com.example.fitplan.Models.Api.ApiManager.addWorkout
    ...
    class CreateWorkoutFragment : Fragment() {
        ...
        private fun setupSaveWorkoutButton(btnSaveWorkout: TextView) {
            ...
            btnSaveWorkout.setOnClickListener {
                ...
                if (exercises.isEmpty()) {
                    Toast.makeText(requireContext(), "Добавьте упражнения", Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                ...
                viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    try {
                        ...
                        val serverUserId = requireContext()
                            .getSharedPreferences("session", 0)
                            .getLong("server_user_id", -1L)

                        if (serverUserId == -1L) {
                            withContext(Dispatchers.Main) {
                                Toast.makeText(requireContext(), "Нет server_user_id", Toast.LENGTH_SHORT).show()
                            }
                            return@launch
                        }
                        ...
                        withContext(Dispatchers.Main) {
                            Toast.makeText(requireContext(), "Тренировка сохранена (сервер)", Toast.LENGTH_SHORT).show()

                            SyncManager.startSync(requireContext()) // 🔥 ДОБАВИЛИ

                            parentFragmentManager.popBackStack()
                        }

                    } catch (e: Exception) {
                        Log.e("SAVE_WORKOUT", "error", e)
        ...
        private fun addExerciseCard(exercise: Exercise) {
            ...
            card.setOnClickListener {
                editingExerciseIndex = exercises.indexOf(exercise)
